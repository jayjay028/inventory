<template>
  <div>
    <PageHeader
      title="Customers"
      :breadcrumbs="[{ label: 'Dashboard', route: '/dashboard' }, { label: 'Customers' }]"
    >
      <template #actions>
        ...
        >
          ...
          <template #actions="{ row }">
            <div class="d-flex gap-1">
              <router-link :to="`/customers/${row.id}/edit`" class="btn btn-sm btn-outline-primary">
...
async function loadData() {
  ...
  try {
    ...
    appStore.showToast('Failed to load customers', 'error')
  } finally {
    loading.value = false
  }
}
...
function toggleStatus(row) {
  ...
  confirmDialog.title = row.active ? 'Deactivate Customer' : 'Activate Customer'
...
async function confirmAction() {
  ...
  try {
    await customersApi.updateStatus(row.id, !row.active)
    appStore.showToast(`Customer ${row.active ? 'deactivated' : 'activated'} successfully`)
    loadData()
  } catch (error) {
    appStore.showToast('Failed to update customer status', 'error')
  }
}
</script>
