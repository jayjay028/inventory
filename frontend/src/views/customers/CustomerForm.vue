...
onMounted(async () => {
  if (isEdit.value) {
    ...
    try {
      const { data } = await customersApi.getById(route.params.id)
      Object.keys(form).forEach(key => {
        form[key] = data[key] || ''
      })
    } catch (error) {
      appStore.showToast('Failed to load customer', 'error')
...
async function handleSubmit() {
  ...
  try {
    if (isEdit.value) {
      ...
      appStore.showToast('Customer updated successfully')
    } else {
      await customersApi.create(form)
      appStore.showToast('Customer created successfully')
    }
    router.push('/customers')
  } catch (error) {
    ...
    if (data?.errors) {
      ...
      appStore.showToast(data?.message || 'Failed to save customer', 'error')
    }
  } finally {
